...
The AUC for the test data is .5179
...
svmbest <- svm(spam ~ inherit + winner, data = d, kernel = "radial", 
    cost = .1, gamma = .5, scale = TRUE)
```

The optimal tuning parameters are for radial gamma .5 and cost .1 with .0935958 performance, 
and with linear gamma .5 and cost .1 with performance of .0935951, making the radial
...
```
The AUC is .528 for the model with the optimal parameters above. 
...
```
This visualization makes it clear that there is a much higher AUC in the logistic model than in the svm model, as seen by the higher AUC value and how the curve is close to the top left corner. 
